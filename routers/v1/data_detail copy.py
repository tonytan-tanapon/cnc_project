...
#     q = (
#         db.query(
#             ProductionLot.id.label("lot_id"),  # 👈 new column
#             ProductionLot.po_id.label("po_id"),  # 👈 add this line
#             lot_no_col,      # 0
#             po_num_col,      # 1
#             qty_col,         # 2
...
#     q = q.outerjoin(ShopTraveler, ShopTraveler.lot_id == ProductionLot.id)
...
#     items: List[DetailRow] = []
#     for (lot_id, po_id, lot_no, po_number, qty, lot_due_date, po_due_date) in rows:
#         items.append(DetailRow(
#             lot_id=lot_id,
#             po_id=po_id,   # 👈 include po_id
...
def data_detail(
    ...
    q = (
        db.query(
            ...
            lot_due_col,
            pol_due_col,
            ShopTraveler.id.label("traveler_id"),   # 👈 new column
        )
        .join(PO, ProductionLot.po_id == PO.id)
        .outerjoin(ShopTraveler, ShopTraveler.lot_id == ProductionLot.id)  # 👈 join traveler
        .filter(
            ProductionLot.part_id == part_id,
            PO.customer_id == customer_id,
        )
    ...
    # revision filters (same)
    if revision_id is not None:
        q = q.filter(ProductionLot.part_revision_id == revision_id)
    elif rev is not None:
        q = (
            q.join(PartRevision, PartRevision.id == ProductionLot.part_revision_id)
             .filter(PartRevision.part_id == part_id, PartRevision.rev == rev)
        )
    ...
    for (lot_id, po_id, lot_no, po_number, qty, lot_due_date, po_due_date, traveler_id) in rows:
        items.append(DetailRow(
            ...
            ship_date=None,
            ship_qty=0.0,
            traveler_id=traveler_id,  # 👈 include traveler id
        ))
    ...
    if revision_id:
        ...
        rev_obj = db.query(PartRevision).filter(PartRevision.part_id == part_id, PartRevision.rev == rev).first()
    else:
        rev_obj = db.query(PartRevision).filter(PartRevision.part_id == part_id, PartRevision.is_current == True).first()
