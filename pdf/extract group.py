...
# ======================================
# 🔥 AUTO FEATURE GROUP (สำคัญมาก)
# ======================================
def make_feature_key(row):
    if pd.isna(row["nominal"]) or pd.isna(row["tolerance"]):
        return None
    return f"{row['feature']}_{row['nominal']}_{row['tolerance']}"

df["feature_key"] = df.apply(make_feature_key, axis=1)

# ======================================
# FILTER VALID FOR CPK
# ======================================
df_valid = df.dropna(subset=["actual", "nominal", "tolerance", "feature_key"])
