...
export function renderTableX(container, rows, {
  ...
  const th = columns.map(c =>
    `<th${c.width?` style="width:${c.width}"`:''}${c.align?` class="t-${c.align}"`:''}>${escapeHtml(c.title||c.key)}</th>`
  ).join('');

  const tr = rows.map((r, i) => {
    const cells = columns.map(c => {
      if (c.key === '__no') return `<td class="t-right">${rowStart + i + 1}</td>`;
      const v = c.render ? c.render(r) : (r[c.key] ?? '');
      return `<td${c.align?` class="t-${c.align}"`:''}>${typeof v==='string'?v:escapeHtml(String(v))}</td>`;
    }).join('');
  ...
  if (onRowClick) {
    container.querySelector('tbody')?.addEventListener('click', (e) => {
      ...
      const id = tr.dataset.id; const row = rows.find(r => String(getRowId(r))===id);
