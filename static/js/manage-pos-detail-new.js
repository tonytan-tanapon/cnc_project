...
function setDirtyClass(row, on) {
  ...
  if (el && el.classList)
    on ? el.classList.add("is-dirty") : el.classList.remove("is-dirty");
}
...
(function injectOnce() {
  ...
  st.textContent = `
    ...
    .link-badge{display:inline-block;padding:2px 6px;border:1px solid #e5e7eb;border-radius:999px;margin:0 4px 4px 0;font-size:.9em;background:#f9fafb}
  `;
  document.head.appendChild(st);
})();

/* ---------- line links helper (hydrate) ---------- */
async function fetchLineLinks(lineId) {
  try {
    const res = await jfetch(`/po-lines/${encodeURIComponent(lineId)}/links`);
    return res || { lots: [], travelers: [] };
  } catch {
    return { lots: [], travelers: [] };
  }
}

async function hydrateLinesWithLinks(rows) {
  const jobs = rows.map(async (r) => {
    if (!r.id) return r;
    const links = await fetchLineLinks(r.id);
    r.lots = Array.isArray(links.lots) ? links.lots : [];
    r.travelers = Array.isArray(links.travelers) ? links.travelers : [];
    // merge single fields (backward-compat)
    if (r.lot_id && r.lot_no && !r.lots.some((x) => x.id === r.lot_id)) {
      r.lots.unshift({ id: r.lot_id, lot_no: r.lot_no });
    }
    if (
      r.traveler_id &&
      r.traveler_no &&
      !r.travelers.some((x) => x.id === r.traveler_id)
    ) {
      r.travelers.unshift({ id: r.traveler_id, traveler_no: r.traveler_no });
    }
    return r;
  });
...
function buildLinePayload(row) {
  ...
  if (row.due_date === "" || row.due_date == null) payload.due_date = null;
  else {
...
async function createLotAndTravelerForRow(row) {
  ...
  try {
    ...
    row.update({
      lot_id: res.lot_id,
      lot_no: res.lot_no,
      traveler_id: res.traveler_id,
      traveler_no: res.traveler_no,
    });

    // แล้ว hydrate ลิงก์ทั้งหมดของแถวนี้
    const links = await fetchLineLinks(d.id);
    row.update({ lots: links.lots || [], travelers: links.travelers || [] });
  ...
  linesTable = new Tabulator(holder, {
    ...
    columns: [
...

      {
        ...
        formatter: (cell) => {
          const d = cell.getData();
          const list = [...(Array.isArray(d.lots) ? d.lots : [])];
          if (d.lot_id && d.lot_no && !list.some((x) => x.id === d.lot_id)) {
            list.unshift({ id: d.lot_id, lot_no: d.lot_no });
          }
          if (!list.length) return "";
          return list
            .map(
              (x) =>
                `<a class="link-badge" href="/static/lot-detail.html?id=${encodeURIComponent(
                  x.id
                )}">${safe(x.lot_no)}</a>`
            )
            .join(" ");
...

      {
        ...
        formatter: (cell) => {
          const d = cell.getData();
          const list = [...(Array.isArray(d.travelers) ? d.travelers : [])];
          if (
            d.traveler_id &&
            d.traveler_no &&
            !list.some((x) => x.id === d.traveler_id)
          ) {
            list.unshift({ id: d.traveler_id, traveler_no: d.traveler_no });
          }
          if (!list.length) return "";
          return list
            .map(
              (x) =>
                `<a class="link-badge" href="/static/traveler-detail.html?id=${encodeURIComponent(
                  x.id
                )}">${safe(x.traveler_no)}</a>`
            )
            .join(" ");
        },
      },
  ...
  if (!d.id) {
    ...
    jfetch(`/pos/${encodeURIComponent(poid)}/lines`, {
      ...
      .catch((e) => {
        ...
        else toast(e?.message || "Create failed", false);
      })
      .finally(() => {
...
async function reloadLines() {
  ...
  try {
    ...
    const hydrated = await hydrateLinesWithLinks(mapped);
    linesTable?.setData(hydrated);
  } catch {
    linesTable?.setData([]);
  }
}
