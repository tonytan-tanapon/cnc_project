...
const managePartDetailUrl = (partId, revId, custId) =>
  `/static/manage-part-detail.html?part_id=${encodeURIComponent(partId ?? "")}` +
...
function initTable(){
  ...
  table = new Tabulator(tableMount, {
    ...
    ajaxRequestFunc: async (_url, _cfg, params) => {
      ...
      const size = showAll ? DEFAULT_PAGE_SIZE : (Number(params.size) || DEFAULT_PAGE_SIZE);

      const q = (inputSearch?.value || '').trim();
      const usp = new URLSearchParams();
      usp.set('page', String(page));
      if (showAll) usp.set('all', '1'); else usp.set('page_size', String(size));
      if (q) usp.set('q', q);
      ...
      const last_page = showAll ? 1 : Math.max(1, Math.ceil((totalItems || rows.length) / size));
    ...
    columns: [
      { title: "No.", field: "_rowno", width: 70, hozAlign: "right", headerHozAlign: "right", headerSort: false,
        formatter: (cell) => {
          ...
          const eff = ps === true ? (totalItems || cell.getTable().getDataCount()) : (ps || DEFAULT_PAGE_SIZE);
          return (cur - 1) * eff + pos;
        },
      },

      { title: "Part No", field: "part_no", minWidth: 150, headerSort: true,
        formatter: (cell) => {
          const d = cell.getData();
          if (!d?.part_id) return safe(cell.getValue() ?? '');
          const href = managePartDetailUrl(d.part_id, d.part_revision_id, d.customer_id);
        ...
        cellClick: (e, cell) => {
          ...
          const d = cell.getData();
          if (!d?.part_id) return;
          location.href = managePartDetailUrl(d.part_id, d.part_revision_id, d.customer_id);
        },
      },

      { title: "Part Name", field: "part_name",minWidth: 150, headerSort: true,},
        
      { title: "Current Stock", minWidth: 150, headerSort: true,      },

      { title: "Rework/WIP", minWidth: 150, headerSort: true,      },
      { title: "Last Update", minWidth: 150, headerSort: true,      },


      { title: "Lot Number", field: "lot_no", minWidth: 140, headerSort: true, headerFilter: "input",
        formatter: (cell) => {
          const d = cell.getData();
          return d?.id ? `<a class="link" href="${lotDetailUrl(d.id)}">${safe(d.lot_no ?? '')}</a>` : safe(cell.getValue() ?? '');
        },
        cellClick: (e, cell) => { e.stopPropagation(); const d=cell.getData(); if(d?.id) location.href=lotDetailUrl(d.id); },
      },

      { title: "PO Number", field: "po_number", minWidth: 140, headerSort: true,
        formatter: (cell) => {
          const d = cell.getData();
          return d?.po_id ? `<a class="link" href="${poDetailUrl(d.po_id)}">${safe(d.po_number ?? '')}</a>` : safe(cell.getValue() ?? '');
        },
        cellClick: (e, cell) => { e.stopPropagation(); const d=cell.getData(); if(d?.po_id) location.href=poDetailUrl(d.po_id); },
      },

      { title: "Customer No", field: "customer_code", minWidth: 130, headerSort: true },

      { title: "PO Date", field: "po_date", width: 180, headerSort: true, formatter: (cell) => fmtDate(cell.getValue()) },

      // 👉 Part No now links to manage-part-detail.html with ids
      
      { title: "Part Rev", field: "part_rev", width: 120, headerSort: true },

      { title: "Prod Qty", field: "prod_qty", width: 120, hozAlign: "right", headerHozAlign: "right",
        formatter: (c)=>{const v=c.getValue(); return v==null?'':Number(v).toLocaleString(undefined,{maximumFractionDigits:3});}
      },
      { title: "Qty PO", field: "qty_po", width: 120, hozAlign: "right", headerHozAlign: "right",
        formatter: (c)=>{const v=c.getValue(); return v==null?'':Number(v).toLocaleString(undefined,{maximumFractionDigits:3});}
      },
    ],
  });
