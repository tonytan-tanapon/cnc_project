...
const DEBUG = false;
const dlog = (...args) => {
  if (DEBUG) console.log("[PO-D]", ...args);
};

const qs = new URLSearchParams(location.search); // qs = query string on url
const poId = qs.get("id"); // ex http://127.0.0.1:9000/static/pos-detail.html?id=54 → poId = "54"

let original = null; // สำหรับ Reset (PO header) { po_number, customer_id, description, ... }
let selectedCustomer = null; // { id, code, name } ที่เลือกจาก autocomplete

const customersDetailUrl = (id) =>
  `/static/customers-detail.html?id=${encodeURIComponent(id)}`; // customer detail page, use at customer link on header
const partDetailUrl = (id) =>
  `/static/part-detail.html?id=${encodeURIComponent(id)}`; // part detail page, use at part link on lines in table
...
// Parameters
// headerOpen = true/false,
// sec = "page-po-detail",  section id to hide/unhide
// showHeaderEditor(), hideHeaderEditor()
let headerOpen = false; // false = form shown, From for edit po table.
function showHeaderEditor() {
  // แสดงฟอร์มแก้ไขส่วนหัว
  const sec = $("page-po-detail"); //section id to hide/unhide
  if (!sec) return;
  sec.hidden = false;
  $("btnHeaderEdit").textContent = "Cancel"; // ปุ่มเปลี่ยนเป็น Cancel
  headerOpen = true;

  // end of set hide/unhide section

  // set autocomplete
  // ปุ่ม Edit PO กดแล้วจะโฟกัสช่องลูกค้าเลย
  // (ถ้า attach autocomplete ตอนโหลดหน้า อาจคำนวณตำแหน่งผิดเพี้ยนได้)
  // 1. ถ้ายังไม่เคย attach autocomplete ให้ attach
  // 2. โฟกัสช่องลูกค้า + ดึงรายการแนะนำ
  // 3. แสดงกล่องรายการแนะนำ + คำนวณตำแหน่ง

  // attach autocomplete ให้ช่องลูกค้าตอนฟอร์มถูกแสดง (หลีกเลี่ยงปัญหาคำนวณตำแหน่งตอน hidden)
  const custInput = $("po_cust"); // input ช่องลูกค้า
  //1. attach autocomplete
  if (custInput && !custInput.dataset.acReady) {
    // ถ้ายังไม่เคย attach autocomplete
    attachCustomerAutocomplete(custInput); // attach autocomplete
    custInput.dataset.acReady = "1"; // ทำเครื่องหมายว่า attach แล้ว
  }
  //2. โฟกัสช่องลูกค้า + ดึงรายการแนะนำ
  if (custInput) {
    // โฟกัสช่องลูกค้า + ดึงรายการแนะนำ
    custInput.focus(); // ไม่ทำก็ได้
    const term = (custInput.value || "").trim();
    fetchCustomerSuggest(term); // ดึงรายการแนะนำ (ถ้า value ว่างจะดึงรายการล่าสุด 10 รายการ)
    ensureCustAcBox(); // สร้างกล่องถ้ายังไม่มี
    positionCustAcBox(custInput); // 3. แสดงกล่องรายการแนะนำ + คำนวณตำแหน่ง
  }
}
...
function hideHeaderEditor() {
  ...
  if (!sec) return;
  if (original) resetForm(); // รีเซ็ตก่อนซ่อน (ถ้ามีข้อมูลเดิม) (กรณีแก้ไขแล้วกด Cancel)
  sec.hidden = true; // ซ่อนฟอร์ม
  $("btnHeaderEdit").textContent = "Edit PO"; // ปุ่มเปลี่ยนเป็น Edit PO
  headerOpen = false; // form hidden
}
...
async function resolveCustomerIdFromCode(codeOrText) {
  ...
  try {
    ...
    const list = Array.isArray(data)
      ? data
      : data?.items || data?.results || data?.data || data?.list || [];
    const exact = (list || []).find(
      (c) => (c.code || "").toUpperCase() === code
    );
    dlog("resolveCustomerIdFromCode", {
      code,
      url,
      listLen: list?.length,
      exact,
...
function renderCustAc(list) {
  ...
  custAcItems = list || [];
  custAcActive = -1;
  if (custAcItems.length === 0 || !custAcTarget) {
    hideCustAc();
    return;
  }
...
function attachCustomerAutocomplete(input) {
  ...
  input.addEventListener("input", () => {
    ...
    if (
      ...
      !term
        .toUpperCase()
        .startsWith((selectedCustomer.code || "").toUpperCase())
    ) {
    ...
    .map((row, idxInRows) => {
      ...
      if (!isEdit) {
        ...
        const partNoCell = partId
          ? `<a href="${partDetailUrl(partId)}" class="link">${escapeHtml(
              String(partNo)
            )}</a>`
        ...
        return `
        <tr data-id="${row.id}">
          <td class="no-col" style="text-align:right">${escapeHtml(
            String(displayNo)
          )}</td>
          <td>${partNoCell}</td>
        ...
        <tr data-id="${row.id ?? ""}" data-editing="1">
          ...
          <td>
            <input id="r_due_${rid}" type="date" value="${escapeHtml(
          String(due)
        )}">
          </td>
          <td>
