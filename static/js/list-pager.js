...
function makePagerUI(pager, { pageInfoEls = [], prevButtons = [], nextButtons = [] } = {}) {
  const infos = (Array.isArray(pageInfoEls) ? pageInfoEls : [pageInfoEls]).filter(Boolean);
  const prevs = (Array.isArray(prevButtons) ? prevButtons : [prevButtons]).filter(Boolean);
  const nexts = (Array.isArray(nextButtons) ? nextButtons : [nextButtons]).filter(Boolean);

  const update = (busy = false) => {
    const text = `Page ${pager.pageIndex}`;
    infos.forEach(el => { el.textContent = text; });
    ...
    prevs.forEach(b => { if (b) { b.disabled = disablePrev; b.setAttribute('aria-disabled', String(disablePrev)); }});
    nexts.forEach(b => { if (b) { b.disabled = disableNext; b.setAttribute('aria-disabled', String(disableNext)); }});
  };
 ...
 * @param {HTMLElement} opts.container      - element ที่จะเรนเดอร์ตาราง
 * @param {(container:HTMLElement, rows:any[])=>void} opts.render
...
export function createListPager(opts) {
  ...
  if (!url) throw new Error('createListPager: opts.url is required');
  if (!container) throw new Error('createListPager: opts.container is required');
  if (typeof render !== 'function') throw new Error('createListPager: opts.render must be a function');
  ...
  async function reloadFirst() {
    ...
    try {
      ...
      const ctx = { rowStart: (pager.pageIndex - 1) * state.pageSize, pageIndex: pager.pageIndex, pageSize: state.pageSize };
  ...
  function bindSearch(inputEl, { debounceMs = 300, normalize = s => s } = {}) {
    if (!inputEl) return () => {};
  ...
  function bindButtons({ prevButtons, nextButtons } = {}) {
    (Array.isArray(prevButtons) ? prevButtons : [prevButtons]).filter(Boolean)
      .forEach(b => b.addEventListener('click', goPrev));
    (Array.isArray(nextButtons) ? nextButtons : [nextButtons]).filter(Boolean)
  ...
  return {
    ...
    // core actions
    reloadFirst, goNext, goPrev,

    // search/filters/perPage
    setQuery, setFilters, replaceFilters, setPageSize,

    // wiring helpers
    bindSearch, bindPerPage,
