        ...
        with CSV_FILE.open("r", encoding=CSV_ENCODING, newline="") as f:
            ...
            for row in reader:
                ...
                if qty_ship == None:
                    ...
                    qty_ship = 0
                print(lot_no,po_number,lot_cal)
                if po_number not in lot_cal:
                    
                    lot_cal[po_number] = max(qty_po,qty_ship)     
                    lot_qty = lot_cal[po_number]           
                               
                else: 
                    print( lot_cal[po_number] ,">", qty_ship)
                    if  lot_cal[po_number] >= qty_ship:
                        lot_qty =  0   
                    else:
                        lot_qty = qty_ship - lot_cal[po_number]
                        lot_cal[po_number] = lot_qty
                
                
                print("    before lot_qty", lot_qty, "lot_cal[po_number]",lot_cal[po_number],"qty po", qty_po, "qty_ship", qty_ship)
                ...
                if qty_ship:
                    ...
                    lot_cal[po_number] = lot_cal[po_number] - qty_ship
                    print("          after lot_qty", lot_cal[po_number],"qty po", qty_po, "qty_ship", qty_ship)
