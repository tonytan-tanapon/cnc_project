...
def qn0(x):  # SQL numeric, but default to 0 if missing
    v = qn(x)
...
for i, r in df.iterrows():
    ...
    total_line  = money_to_decimal(g("Total"))  # not used downstream, but parsed for completeness
    fair        = s(g("FAIR"))
...
SELECT po.id, p.id,
       ...
       {qn0(qty_po)},                     -- <== was qn(qty_po); now defaults to 0
       {qn0(price)},                      -- <== optional: default price to 0 too
       {qd(due_date)}, {q(notes)}
...
""".strip())
    ...
    # PRODUCTION LOT  (must include part_id, part_revision_id, po_id!)
    if lot_no:
        if rev:
            emit(f"""
...
""".strip())
    ...
    # CUSTOMER SHIPMENT + ITEM (one shipment per row; idempotent by date + LOT note)
...
print(f"[OK] wrote SQL -> {OUTPUT_SQL}")
