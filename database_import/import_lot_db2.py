...
def is_not_number(value):
    try:
        ...
        return False   # เป็นตัวเลข
    except (ValueError, TypeError):
         return True    # ไม่ใช่ตัวเลข
...
def safe_int(value, default=0):
    try:
        ...
        if isinstance(value, str):
            value = value.strip()
            if value == "" or value.upper() in {"#VALUE!", "#N/A", "N/A"}:
                return default
...
def main():
    ...
    with SessionLocal() as db:
        ...
        with CSV_FILE.open("r", encoding=CSV_ENCODING, newline="") as f:
            ...
            processed = 0
            for row in reader:
                # print("Processing row:", processed+1)
                ## add customers
                ...
                qty_ship = parse_int(all_lots.get(lot_no, {}).get("qty_ship", 0))  
                if lot_no == "L17191":
                    print("Lot No:", lot_no, "PO No:", po_number, "Qty PO:", qty_po, "Qty Ship:", qty_ship)
                # --- Part ---
                ...
                if po_number == "5076D151-1":
                    print("Processing PO:", po_number, "Lot No:", lot_no, "Qty PO:", qty_po, "Qty Ship:", qty_ship)
                if not po:
                    po = PO(
                        po_number=po_number, 
                ...
                lot_qty = 0
                if  all_lots.get(lot_no, {}).get("qty_ship", 0) != None and all_lots.get(lot_no, {}).get("good_sotck", 0) != None :
                    # print("Processing Lot:",  all_lots.get(lot_no, {}).get("qty_ship", 0),  all_lots.get(lot_no, {}).get("good_sotck", 0))
                    lot_qty = safe_int(all_lots.get(lot_no, {}).get("qty_ship", 0)) + safe_int(all_lots.get(lot_no, {}).get("good_sotck", 0))
                else :
                    lot_qty = all_lots.get(lot_no, {}).get("prod_qty", 0)
                # print("lot_qty from excel:", lot_no, lot_qty,qty_ship, qty_po)
        
                # print("Processing Lot:", lot_no, "lot_qty:", lot_qty, "qty_ship:", qty_ship, "qty_po:", qty_po)
                if is_not_number(lot_qty) :
                # --- Lot ---
                    lot_qty = 0
                lot = get_or_upsert_lot(
                    ...
                    note=need_remark,
                    fair_note=fair_no,
                    lot_po_date=to_utc_midnight(due_date) if due_date else None,
                )
...
if __name__ == "__main__":
    ...
    print("Lot data extracted from Excel.")
    main()
