-- =========================================================
-- Topnotch MFG – Initial Schema v001 (with employees & lot QR support)
-- PostgreSQL
-- =========================================================
BEGIN;
...
CREATE TABLE employees (
  ...
  employee_code   TEXT UNIQUE,          -- รหัสพนักงานในโรงงาน (ถ้ามี)
  first_name      TEXT NOT NULL,
  last_name       TEXT NOT NULL,
  position        TEXT,                 -- ตำแหน่ง (Operator, QA, Shipping)
...
-- ========== 4) Production Lots (with QR support via separate table) ==========
CREATE TABLE lots (
  id                      BIGSERIAL PRIMARY KEY,
...
-- ตัวเลือกเก็บข้อมูล QR แยกจาก lots (แนะนำให้เก็บ payload/URL มากกว่าเก็บรูป)
CREATE TABLE lot_qr_codes (
  id             BIGSERIAL PRIMARY KEY,
  lot_id         BIGINT NOT NULL UNIQUE REFERENCES lots(id) ON DELETE CASCADE,
  payload        TEXT NOT NULL,         -- ข้อความ/URL/JSON ที่จะ encode เป็น QR (เช่น "LOT:L16899-1")
  format         TEXT CHECK (format IN ('text','url','json')) DEFAULT 'text',
  image_url      TEXT,                  -- ถ้า generate แล้วเก็บรูปไว้ที่ object storage; เก็บลิงก์ไว้ได้
...
CREATE TABLE workflow_step_instances (
  ...
  completed_at             TIMESTAMPTZ,
  assignee_employee_id     BIGINT REFERENCES employees(id), -- แทนข้อความชื่อ
